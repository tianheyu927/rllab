...
from multiworld.envs.mujoco.sawyer_xyz.sawyer_pick_and_place_mil_vision import SawyerPickPlaceMILVisionEnv
from multiworld.core.flat_goal_env import FlatGoalEnv

# DEMO_DIR = 'data/sawyer_pick_and_place_noisy_larger_textures_goalpos/'#_push/'
# DEMO_DIR = 'data/sawyer_pick_and_place_noisy_larger_textures_goalpos_push_scripted/'#_push/'
# DEMO_DIR = 'data/sawyer_pick_and_place_noisy_larger_textures_goalpos_initobj/'#_push/'
DEMO_DIR = 'data/sawyer_pick_and_place_noisy_larger_textures_goalpos_push_firstobj_test/'#_push/'
...
# TEST_DEMO_DIR = 'data/sawyer_pick_and_place_noisy_larger_textures_goalpos_push_initobj_test/'#_push/'
...
META_PATH = '/home/kevin/maml_imitation_private/data/checkpoints/sawyer_pick_and_place_textures_goalpos_push.xavier_init.4_conv.2_strides.24_5x5_filters.bt_dim_20.mbs_10.ubs_1.meta_lr_0.001.numstep_1.updatelr_0.05.3_fc.200_dim.fp.learn_ee_pos.eps_10.two_heads.1d_conv_act_3_64_ee_3_64_10x1_filters/model_67000.meta'
LOG_DIR = '/home/kevin/maml_imitation_private/data/checkpoints/sawyer_pick_and_place_textures_goalpos_push.xavier_init.4_conv.2_strides.24_5x5_filters.bt_dim_20.mbs_10.ubs_1.meta_lr_0.001.numstep_1.updatelr_0.05.3_fc.200_dim.fp.learn_ee_pos.eps_10.two_heads.1d_conv_act_3_64_ee_3_64_10x1_filters/'
# META_PATH = '/home/kevin/maml_imitation_private/data/checkpoints/sawyer_pick_and_place_noisy_larger_textures_goalpos_push_initobj.xavier_init.4_conv.2_strides.24_5x5_filters.bt_dim_20.mbs_10.ubs_1.meta_lr_0.001.numstep_1.updatelr_0.05.3_fc.200_dim.conv_bt.all_fc_bt.fp.learn_ee_pos.eps_15.two_heads.final_ee_two_heads/model_88000.meta'
# LOG_DIR = '/home/kevin/maml_imitation_private/data/checkpoints/sawyer_pick_and_place_noisy_larger_textures_goalpos_push_initobj.xavier_init.4_conv.2_strides.24_5x5_filters.bt_dim_20.mbs_10.ubs_1.meta_lr_0.001.numstep_1.updatelr_0.05.3_fc.200_dim.conv_bt.all_fc_bt.fp.learn_ee_pos.eps_15.two_heads.final_ee_two_heads/'
MAX_PATH_LENGTH = 115
...
def load_env(xml_file=None, **kwargs):
	...
	if '4distr' in xml_file:
		...
		n_distractors = 2
		hand_pos_is_init = False
	baseEnv = SawyerPickPlaceMILVisionEnv(xml_file=xml_file, include_distractors=True, n_distractors=n_distractors, random_hand_init_pos=False, hand_pos_is_init=hand_pos_is_init, **kwargs)
...
def eval_success(path):
	  ...
	  if np.amin(infos['dropDistx'][-5:]) < 0.1 and np.amin(infos['dropDisty'][-5:]) < 0.1:# and np.amin(infos['handGoalDist'][-5:]) < 0.08:
		  return True
	  else:
		  return False
...
def main(meta_path, demo_dir, log_dir, validation=False, save_video=True, lstm=False, num_input_demos=1):
	...
	with tf.Session(config=tf_config) as sess:
		...
		num_success = 0
		num_trials = 0
		trials_per_task = 3 #10 #3
		
		if validation:
			# task_ids = all_ids[-115:-96]#[-119:-99]#[1:2]#[-25:]
			# task_ids = all_ids[-70:]
			# task_ids = all_ids[-126:-106]
			task_ids = all_ids[:10] + all_ids[20:30]
			# task_ids = all_ids[20:40]
		else:
			task_ids = all_ids
...
# 			import pdb; pdb.set_trace()
			...
			# for j in range(4, 4+trials_per_task+1):
				...
				print('Actual final eept is', init_objPos)
				...
				if eval_success(path):
					num_success += 1
				print('Final eept loss is', (np.abs(init_objPos[:-1] - path['final_eept']) + 0.01*((init_objPos[:-1] - path['final_eept'])**2)).mean())
...
if __name__ == '__main__':
	...
	main(meta_path=META_PATH, demo_dir=demo_dir, log_dir=LOG_DIR, validation=validation, lstm=lstm, save_video=False)
